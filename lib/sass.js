...
function parse(tokens) {
  ...
  /**
   * Cloning required for mixin (selector can have only one parent)
   */
  function clone(selector, parent){
    var result = new Selector(selector.string, parent, selector.type)
    result.properties = selector.properties
...
/**
 * Replace @import directive in _sass_ by referenced _sass_.
 * Returns string containing _sass_ with resolved imports.
 *
 * @param  {string} sass
 * @return {string}
 * @api private
...
function import(sass, options) {
  var capture, str = sass, partial = ''
  while(capture = importExpression.exec(str)){
	partial = options.locals.partial((capture[1] || capture[2]), {inline: true});
    str = str.replace(importExpression, partial)
  }  
  return str;
}

 var importExpression = /@import (?:'(.*?)'|"(.*?)")/; // /@import '(.*?)'/;
...
exports.render = function(sass, options) {
  ...
  return compile(exports.collect(import(sass, options)).selectors)
}
